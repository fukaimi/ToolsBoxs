<template>
  <a-layout id="components-layout-demo-custom-trigger">
    <a-layout-sider v-model="collapsed" :trigger="null" collapsible >
      ...
      <a-menu theme="dark" mode="inline" :default-selected-keys="[0]" style="overflow-y:scroll;height:90vh;">
        <a-sub-menu v-for="(item,index) in mainMenu" :key="index" @titleClick="titleClick">
          <span slot="title"><a-icon :type="item.icon" /><span>{{ item.name }}</span></span>
          <a-menu-item-group :key="item.id">
            <a-menu-item v-for="ite in item.param" :key="ite.pageName" @click="menuHandle(ite)">
              <a-icon :type="ite.icon" />
              <span>{{ ite.title }}</span>
            </a-menu-item>
          </a-menu-item-group>
        </a-sub-menu>
    ...
    <a-layout>
      ...
      <a-layout-content
        style="margin: 24px 16px; padding: 24px; background: #fff; height:80vh;overflow-y:scroll"
      >
        <router-view/>
      </a-layout-content>
    </a-layout>
